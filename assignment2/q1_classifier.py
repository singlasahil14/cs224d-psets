...
class SoftmaxModel(Model):
  ...
  def add_placeholders(self):
    ...
    self.labels_placeholder = tf.placeholder(tf.int32, shape=(batch_size, n_classes))
    ### END YOUR CODE
  ...
  def add_model(self, input_data):
    ...
    with tf.name_scope("linear_softmax"):
      self.W = tf.Variable(tf.zeros([n_features, n_classes]), dtype=tf.float32, name='weights')
      self.b = tf.Variable(tf.zeros([n_classes]), dtype=tf.float32, name='biases')
    logits = tf.matmul(input_data, self.W) + self.b
    out = softmax(logits)
    ### END YOUR CODE
    return out
