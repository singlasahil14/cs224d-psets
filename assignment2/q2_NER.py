...
class Config(object):
  ...
  def __str__(self):
    config_dict = {}
    config_dict['embed_size'] = self.embed_size
    config_dict['batch_size'] = self.batch_size
    config_dict['label_size'] = self.label_size
    config_dict['hidden_size'] = self.hidden_size
    config_dict['max_epochs'] = self.max_epochs
    config_dict['early_stopping'] = self.early_stopping
    config_dict['dropout'] = self.dropout
    config_dict['lr'] = self.lr
    config_dict['l2'] = self.l2
...
class NERModel(LanguageModel):
  ...
  def add_placeholders(self):
    ...
    self.labels_placeholder = tf.placeholder(tf.float32, shape=(None, label_size))
  ...
  def add_model(self, window):
    ...
    with tf.variable_scope('hidden') as scope:
      self.W = tf.get_variable(name='weights', shape=[window_size*embed_size, hidden_size],
                               initializer=xavier_weight_init())
      self.b1 = tf.get_variable(name='biases', shape=[hidden_size],
                                initializer=tf.constant_initializer(0.0))
      hidden = tf.tanh(tf.matmul(window, self.W) + self.b1)
    with tf.variable_scope('softmax') as scope:
      self.U = tf.get_variable(name='weights', shape=[hidden_size, label_size],
                               initializer=xavier_weight_init())
  ...
  def add_loss_op(self, y):
    ...
    """
    ### YOUR CODE HERE
    self.cross_entropy_loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(y, self.labels_placeholder))
    tf.add_to_collection("total_loss", self.cross_entropy_loss)
